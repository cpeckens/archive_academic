 ...
 * Define Constants
 */
define('KSASACA_SHORTNAME', 'ksasaca'); // used to prefix the individual setting field id see ksasaca_options_page_fields()
define('KSASACA_PAGE_BASENAME', 'ksasaca-settings'); // the settings page slug
 ...
 * @return array
 */
function ksasaca_get_settings() {
	
	$output = array();
	
	// put together the output array 
	$output['ksasaca_option_name'] 		= 'ksasaca_options'; // the option name as used in the get_option() call.
	$output['ksasaca_page_title'] 		= __( 'Academic Theme Options','ksasaca_textdomain'); // the settings page title
	$output['ksasaca_page_sections'] 	= ksasaca_options_page_sections(); // the setting section
	$output['ksasaca_page_fields'] 		= ksasaca_options_page_fields(); // the setting fields
 ...
 */
function ksasaca_create_settings_field( $args = array() ) {
	...
	$defaults = array(
		...
		'section' => 'main_section', 					// the section this setting belongs to — must match the array key of a section in ksasaca_options_page_sections()
	...
	// additional arguments for use in form field output in the function ksasaca_form_field_fn!
	$field_args = array(
		'type'      => $type,
		'id'        => $id,
		'desc'      => $desc,
	...
	add_settings_field( $id, $title, 'ksasaca_form_field_fn', __FILE__, $section, $field_args );
 ...
 */
function ksasaca_register_settings(){
	
	// get the settings sections array
	$settings_output 	= ksasaca_get_settings();
	$ksasaca_option_name = $settings_output['ksasaca_option_name'];
	
	//setting
	register_setting($ksasaca_option_name, $ksasaca_option_name, 'ksasaca_validate_options' );
	
	//sections
	if(!empty($settings_output['ksasaca_page_sections'])){
		// call the "add_settings_section" for each!
		foreach ( $settings_output['ksasaca_page_sections'] as $id => $title ) {
			add_settings_section( $id, $title, 'ksasaca_section_fn', __FILE__);
		}
	}
		
	//fields
	if(!empty($settings_output['ksasaca_page_fields'])){
		// call the "add_settings_field" for each!
		foreach ($settings_output['ksasaca_page_fields'] as $option) {
			ksasaca_create_settings_field($option);
		}
	}
}
 ...
 */
function  ksasaca_section_fn($desc) {
	echo "<p>" . __('Settings for this section','ksasaca_textdomain') . "</p>";
}
 ...
 * @return echoes output
 */
function ksasaca_form_field_fn($args = array()) {
	...
	$ksasaca_option_name = $settings_output['ksasaca_option_name'];
	$options 			= get_option($ksasaca_option_name);
	...
	switch ( $type ) {
		case 'text':
			...
			echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $ksasaca_option_name . "[$id]' value='$options[$id]' />";
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				$item[0] = esc_html__($item[0], 'ksasaca_textdomain');
				if (!empty($options[$id])) {
				...
				echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $ksasaca_option_name . "[$id|$item[1]]' value='$value' /><br/>";
			}
		...
		case 'textarea':
			...
			echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $ksasaca_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
		
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $ksasaca_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'ksasaca_textdomain');
					$item 	= esc_html($item, 'ksasaca_textdomain');
		...
		case 'select2':
			...
			foreach($choices as $item) {
				...
				$item = explode("|",$item);
				$item[0] = esc_html($item[0], 'ksasaca_textdomain');
		...
		case 'checkbox':
			echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $ksasaca_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
		...
		case "multi-checkbox":
			foreach($choices as $item) {
				...
				echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $ksasaca_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
			}
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
	}
 ...
 * @return echoes output
 */
function ksasaca_settings_page_fn() {
	// get the settings sections array
	$settings_output = ksasaca_get_settings();
?>
	<div class="wrap">
		<div class="icon32" id="icon-options-general"></div>
		<h2><?php echo $settings_output['ksasaca_page_title']; ?></h2>
 ...
 * @return array
 */
function ksasaca_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['ksasaca_page_fields'];
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									KSASACA_SHORTNAME . '_txt_numeric_error', // error ID
									__('Expecting a Numeric value! Please fix.','ksasaca_textdomain'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','ksasaca_textdomain');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									$option['id'], // setting title
									KSASACA_SHORTNAME . '_txt_multinumeric_error', // error ID
									__('Expecting comma separated numeric values! Please fix.','ksasaca_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','ksasaca_textdomain');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','ksasaca_textdomain');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									$option['id'], // setting title
									KSASACA_SHORTNAME . '_txt_email_error', // error ID
									__('Please enter a valid email address.','ksasaca_textdomain'), // error message
									'error' // type of message
								);
							}
						break;
 ...
 */
function ksasaca_show_msg($message, $msgclass = 'info') {
